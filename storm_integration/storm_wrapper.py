...
class StormWrapper:
    ...
    def _build_content_prompt(
        ...
        if self.language == "id":
            ...
            prompt += "   - STUDI KASUS HARUS SANGAT DETAIL DAN BERCERITA:\n"
            prompt += "     * JANGAN hanya 1 kalimat per poin - setiap bagian harus dijelaskan dengan paragraf penuh\n"
            prompt += "     * Gunakan format naratif yang bercerita, bukan hanya list poin-poin\n"
            prompt += "     * Context: Ceritakan latar belakang situasi dengan detail (2-3 paragraf)\n"
            prompt += "       - Siapa yang terlibat, kapan terjadi, di mana, situasi seperti apa\n"
            prompt += "       - Kondisi awal, tantangan yang dihadapi, mengapa ini penting\n"
            prompt += "     * Problem: Jelaskan masalah dengan sangat detail (2-3 paragraf)\n"
            prompt += "       - Apa masalahnya, mengapa muncul, dampaknya, kompleksitasnya\n"
            prompt += "       - Mengapa solusi tradisional tidak cukup, apa yang dibutuhkan\n"
            prompt += "     * Solution: Ceritakan solusi secara detail dan bertahap (3-4 paragraf)\n"
            prompt += "       - Bagaimana solusi dirancang, langkah-langkah implementasinya\n"
            prompt += "       - Teknologi/metode yang digunakan, mengapa dipilih\n"
            prompt += "       - Proses pengembangan, tantangan teknis yang dihadapi\n"
            prompt += "     * Results: Jelaskan hasil dengan detail dan data (2-3 paragraf)\n"
            prompt += "       - Apa yang dicapai, metrik keberhasilan, perbandingan sebelum/sesudah\n"
            prompt += "       - Dampak jangka pendek dan jangka panjang\n"
            prompt += "       - Keberhasilan dan juga keterbatasan yang ditemui\n"
            prompt += "     * Lessons Learned: Refleksi mendalam (2-3 paragraf)\n"
            prompt += "       - Apa yang dipelajari, insight penting, best practices\n"
            prompt += "       - Apa yang akan dilakukan berbeda, rekomendasi untuk kasus serupa\n"
            prompt += "     * Total minimal 10-15 paragraf untuk setiap studi kasus lengkap\n"
            prompt += "     * Buat pembaca merasa seperti membaca cerita yang menarik dan informatif\n"
            ...
            prompt += "   - WAJIB GUNAKAN MERMAID DIAGRAM untuk berbagai jenis visualisasi:\n"
            prompt += "     * Flowchart (flowchart LR/TD) untuk proses, alur kerja, atau decision flow\n"
            prompt += "     * Sequence diagram (sequenceDiagram) untuk interaksi atau komunikasi\n"
            prompt += "     * Graph (graph LR/TD) untuk hubungan, hierarki, atau struktur\n"
            prompt += "     * State diagram (stateDiagram-v2) untuk state machine atau lifecycle\n"
            prompt += "     * Quadrant diagram (quadrantChart) untuk memetakan konsep dalam 4 kuadran\n"
            prompt += "       - Contoh: memetakan tools berdasarkan complexity vs value\n"
            prompt += "       - Format: quadrantChart\n"
            prompt += "     * C4 diagram (C4Context, C4Container, C4Component) untuk arsitektur sistem\n"
            prompt += "       - C4Context untuk context diagram tingkat tinggi\n"
            prompt += "       - C4Container untuk container diagram\n"
            prompt += "       - C4Component untuk component diagram\n"
            prompt += "     * Block diagram (block) untuk menunjukkan struktur blok atau modul\n"
            prompt += "       - Format: block untuk diagram blok sederhana\n"
            prompt += "     * Architecture diagram (architecture) untuk arsitektur sistem yang kompleks\n"
            prompt += "       - Format: architecture untuk diagram arsitektur\n"
            prompt += "     * Minimal 2-4 diagram mermaid per chapter (variasi jenis diagram)\n"
            prompt += "   - Format mermaid: gunakan ```mermaid di awal dan ``` di akhir\n"
            prompt += "   - Pilih jenis diagram yang paling sesuai dengan konten yang dijelaskan\n"
            ...
            prompt += "- Studi kasus yang hanya 1 kalimat per bagian (Context, Problem, Solution, dll)\n"
            prompt += "- Studi kasus yang hanya berupa list tanpa narasi yang bercerita\n"
            ...
            prompt += "7. FORMAT STUDI KASUS:\n"
            prompt += "   - Gunakan heading '## Case Study: [Judul]' atau '## Studi Kasus: [Judul]' untuk setiap studi kasus\n"
            prompt += "   - Tulis setiap bagian (Context, Problem, Solution, Results, Lessons Learned) sebagai paragraf penuh\n"
            prompt += "   - Bercerita - buat menarik dan informatif, seperti membaca cerita yang menarik\n"
            prompt += "   - Gunakan bahasa deskriptif dan detail spesifik\n"
            prompt += "   - Hubungkan setiap bagian dengan transisi yang halus\n"
            prompt += "   - Buat pembaca memahami tidak hanya apa yang terjadi, tapi juga mengapa dan bagaimana\n"
            prompt += "   - Minimal 10-15 paragraf untuk setiap studi kasus lengkap\n\n"
            ...
            prompt += "   - CASE STUDIES MUST BE VERY DETAILED AND NARRATIVE:\n"
            prompt += "     * DO NOT use only 1 sentence per point - each section must be explained with full paragraphs\n"
            prompt += "     * Use narrative format that tells a story, not just bullet points\n"
            prompt += "     * Context: Tell the background situation in detail (2-3 paragraphs)\n"
            prompt += "       - Who was involved, when it happened, where, what the situation was\n"
            prompt += "       - Initial conditions, challenges faced, why this is important\n"
            prompt += "     * Problem: Explain the problem in great detail (2-3 paragraphs)\n"
            prompt += "       - What the problem was, why it arose, its impact, its complexity\n"
            prompt += "       - Why traditional solutions weren't enough, what was needed\n"
            prompt += "     * Solution: Tell the solution in detail and step-by-step (3-4 paragraphs)\n"
            prompt += "       - How the solution was designed, implementation steps\n"
            prompt += "       - Technologies/methods used, why they were chosen\n"
            prompt += "       - Development process, technical challenges faced\n"
            prompt += "     * Results: Explain results with detail and data (2-3 paragraphs)\n"
            prompt += "       - What was achieved, success metrics, before/after comparison\n"
            prompt += "       - Short-term and long-term impact\n"
            prompt += "       - Successes and also limitations encountered\n"
            prompt += "     * Lessons Learned: Deep reflection (2-3 paragraphs)\n"
            prompt += "       - What was learned, important insights, best practices\n"
            prompt += "       - What would be done differently, recommendations for similar cases\n"
            prompt += "     * Total minimum 10-15 paragraphs for each complete case study\n"
            prompt += "     * Make readers feel like they're reading an engaging and informative story\n"
            ...
            prompt += "   - MUST USE MERMAID DIAGRAMS for various types of visualizations:\n"
            prompt += "     * Flowchart (flowchart LR/TD) for processes, workflows, or decision flows\n"
            prompt += "     * Sequence diagram (sequenceDiagram) for interactions or communication\n"
            prompt += "     * Graph (graph LR/TD) for relationships, hierarchies, or structures\n"
            prompt += "     * State diagram (stateDiagram-v2) for state machines or lifecycles\n"
            prompt += "     * Quadrant diagram (quadrantChart) for mapping concepts in 4 quadrants\n"
            prompt += "       - Example: mapping tools by complexity vs value\n"
            prompt += "       - Format: quadrantChart\n"
            prompt += "     * C4 diagram (C4Context, C4Container, C4Component) for system architecture\n"
            prompt += "       - C4Context for high-level context diagram\n"
            prompt += "       - C4Container for container diagram\n"
            prompt += "       - C4Component for component diagram\n"
            prompt += "     * Block diagram (block) for showing block or module structure\n"
            prompt += "       - Format: block for simple block diagrams\n"
            prompt += "     * Architecture diagram (architecture) for complex system architecture\n"
            prompt += "       - Format: architecture for architecture diagrams\n"
            prompt += "     * Minimum 2-4 mermaid diagrams per chapter (vary diagram types)\n"
            prompt += "   - Mermaid format: use ```mermaid at start and ``` at end\n"
            prompt += "   - Choose diagram type that best fits the content being explained\n"
            ...
            prompt += "- Case studies with only 1 sentence per section (Context, Problem, Solution, etc.)\n"
            prompt += "- Case studies that are just lists without narrative storytelling\n"
            ...
            prompt += "7. FORMAT CASE STUDY:\n"
            prompt += "   - Use heading '## Case Study: [Title]' for each case study\n"
            prompt += "   - Write each section (Context, Problem, Solution, Results, Lessons Learned) as full paragraphs\n"
            prompt += "   - Tell a story - make it engaging and informative\n"
            prompt += "   - Use descriptive language and specific details\n"
            prompt += "   - Connect each section smoothly with transitions\n"
            prompt += "   - Make readers understand not just what happened, but why and how\n\n"
